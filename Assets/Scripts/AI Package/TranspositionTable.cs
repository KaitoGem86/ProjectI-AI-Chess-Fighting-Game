namespace AIPackage{
	...
	public class TranspositionTable<T> where T : Action {
		...
		public TranspositionTable (Enviroment<T> board, int size) {
			this.board = board;
			this.size = (ulong) size;

			entries = new Entry<T>[size];
		}
		...
		public int LookupEvaluation (int depth, int plyFromRoot, int alpha, int beta) {
			...
			if (entry.key == board.ZobristKey) {
				...
				if (entry.depth >= depth) {
					int correctedScore = CorrectRetrievedWinGameScore (entry.value, plyFromRoot);
		...
		public void StoreEvaluation (int depth, int numPlySearched, int eval, int evalType, T move) {
			if (!enabled) {
				return;
			}
			//ulong index = Index;
			//if (depth >= entries[Index].depth) {
			Entry<T> entry = new Entry<T> (board.ZobristKey, CorrectWinGameScoreForStorage (eval, numPlySearched), (byte) depth, (byte) evalType, move);
			entries[Index] = entry;
			//}
		}

		int CorrectWinGameScoreForStorage (int score, int numPlySearched) {
			if (Search<T>.IsWinGameScore (score)) {
		...
		int CorrectRetrievedWinGameScore (int score, int numPlySearched) {
			if (Search<T>.IsWinGameScore (score)) {
				int sign = System.Math.Sign (score);
				return (score * sign - numPlySearched) * sign;
			}
        ...
        public struct Entry<U> where U : Action{
			...
			public Entry (ulong key, int value, byte depth, byte nodeType, T move) {
				this.key = key;
				this.value = value;
			...
			public static int GetSize () {
				return System.Runtime.InteropServices.Marshal.SizeOf<Entry<U>> ();
			}
		}
    }
}