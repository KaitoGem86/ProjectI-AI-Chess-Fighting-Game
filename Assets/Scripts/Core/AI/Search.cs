	...
	public class Search { // class này cần phải được viết lại một cách tổng quát, Không cần viết lại hàm QuiescenceSearch
		...
		public void StartSearch () {
			...
			if (settings.useIterativeDeepening) {
				int targetDepth = (settings.useFixedDepthSearch) ? settings.depth : int.MaxValue; // điểm cải thiện thuật toán alpha beta prunning => bằng cách sử dụng tìm kiếm sâu dần.
		...
		int SearchMoves (int depth, int plyFromRoot, int alpha, int beta) {
			...
			if (plyFromRoot > 0) {
				...
				if (board.RepetitionPositionHistory.Contains (board.ZobristKey)) { // chuyển hàm này sang board
					return 0;
				}
				...
				// beta can't  possibly be btroetter) than being mated in the current position.
